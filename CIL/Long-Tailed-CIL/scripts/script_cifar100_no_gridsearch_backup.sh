...
do
  if [ "$3" = "base" ]; then
        ...
        PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp-name no_gs_fixd_${SEED} \
                ...
                --pl-classification 1e-4 --pl-mask-entropy 1e-4 --l-reg 1e-3 # LTIO
#                --pl-classification 0.1 --pl-mask-entropy 1e-6 --l-reg 1e-3 # LT
        ...
        PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp-name no_gs_fixd_${SEED} \
                ...
                --num-exemplars-per-class 20 --exemplar-selection herding \
                --pl-classification 1e-3 --pl-mask-entropy 1e-3 --l-reg 5e-3 # LTIO
#                --pl-classification 1e-3 --pl-mask-entropy 1e-3 --l-reg 1e-3 # LT
  else
