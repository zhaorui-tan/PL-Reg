...
class MIRO(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, **kwargs):
        ...
        self.use_partial = hparams.use_partial
        ...
        hat_y_parameters = [
            {"params": self.hat_y_confidence_estimator.parameters(), "lr": hparams.lr * hparams.lr_mult},
            ]
        ...
        self.partial_mask_1 = nn.Linear(3024, 3024, bias=False)
        self.partial_mask_2 = nn.Linear(3024, 3024, bias=False)
        self.sigmoid_1 = nn.Sigmoid()
        self.sigmoid_2 = nn.Sigmoid()
        # nn.init.ones_(self.partial_mask_1.weight.data)
        # nn.init.ones_(self.partial_mask_2.weight.data)
        # nn.init.uniform_(self.partial_mask_1.weight.data, a=0.0, b=1.0)

        # nn.init.uniform_(self.partial_mask_2.weight.data, a=0.0, b=1.0)

        # self.p_relu
        self.partial_logic = nn.Sequential(
            # nn.Linear(num_features, num_features, bias=False),
            nn.Linear(3024, 2, bias=False)
        )

    def update(self, all_x, all_y, feat, inter_feats, loss, confidence_loss, gd_loss_0, y_unseen_confidence, idx,
               idx_unseen, step_ratio):
        all_loss = 0.
        all_loss += loss
        returns = {}
    ...
    def forward(self, x, y=None, mask_range=-1, **kwargs):
        all_x = torch.cat(x)
        all_y = torch.cat(y)
        ...
        if self.low_degree > 0:
            mask = self.partial_mask_1(feat)
            mask = self.sigmoid_1(mask)
            mask = self.partial_mask_2(mask)
            mask = self.sigmoid_2(mask)

            if self.use_partial > 0:
                feat = feat * mask
                p_feat = feat * (1. - mask)

                PL_true = self.partial_logic(feat)
                PL_False = self.partial_logic(p_feat)
                ...
                PL_loss = nn.CrossEntropyLoss()(pl, pl_target) * 1. * self.low_degree * self.use_partial
                # returns['PL_loss'] = PL_loss.item()
                loss += PL_loss
                ...
                loss += -mask_entropy * 0.5 * self.low_degree  # * self.use_partial
                # returns['mask_entropy'] = -mask_entropy.item()
            else:
                feat = feat
