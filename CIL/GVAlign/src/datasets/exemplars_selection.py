    ...
    def _select_indices(self, model: LLL_Net, sel_loader: DataLoader, exemplars_per_class: int, transform) -> Iterable:
        ...
        with torch.no_grad():
            ...
            for images, targets in sel_loader:
                feats = model(images.to(model_device), return_features=True, return_PL=False)[1]

                if type(feats) is dict:
                    # check for PODNet case
        ...
        for curr_cls in np.unique(extracted_targets):
            ...
            for k in range(cls_sample):
                ...
                if 'newonefeat' not in locals():
                    # print as much debug as you can:
                    print("`newonefeat` variable not set, printing current memory state: \n")
