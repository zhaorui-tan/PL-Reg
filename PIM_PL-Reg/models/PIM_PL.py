...
class PIM_partitioner(nn.Module):
    def __init__(self, num_features=512, num_classes=100, temp=25,  r=1.):
        ...
        self.partial_mask_1 = nn.Linear(num_features, num_features, bias=False)
        self.partial_mask_2 = nn.Linear(num_features, num_features, bias=False)
    ...
    def forward(self, x, y=None, mb_lab_mask=None,):
        ...
        PL_true = self.partial_logic(x)
        PL_False = self.partial_logic(p_x)

        # out = F.relu(out)
        # return out, out1

        # partial logical
        return x, out, PL_true, PL_False, mask