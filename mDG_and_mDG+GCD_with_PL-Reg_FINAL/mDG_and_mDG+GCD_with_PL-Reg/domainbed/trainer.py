...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None,mask_range=-1, log_save_path=None):
    logger.info("")
    ...
    batch_sizes = np.full([n_envs], hparams["batch_size"], dtype=np.int_)
    ...
    algorithm.cuda()
    # algorithm = nn.DataParallel(algorithm, device_ids=[0, 1, 2]).cuda()
    ...
    for step in range(n_steps):
        ...
        inputs = {**batches, "step": step,'mask_range': mask_range}
        # step_vals = algorithm.update(**inputs)
        all_x, all_y, feat, inter_feats, loss, confidence_loss, gd_loss_0,  y_unseen_confidence, idx, idx_unseen = algorithm(**inputs)
        step_vals = algorithm.update(all_x, all_y,  feat, inter_feats, loss, confidence_loss, gd_loss_0, y_unseen_confidence, idx, idx_unseen, step/n_steps)
        ...
        if step % checkpoint_freq == 0:
            ...
            if results_keys != last_results_keys:
                logger.info(misc.to_row(results_keys, colwidth=30))
                last_results_keys = results_keys
            logger.info(misc.to_row([results[key] for key in results_keys], colwidth=30))
            ...
            if step >= 0:
                ...
                filename = "TE{}_{}_{}.pth".format(test_env_str, step, args.setting_name)
                if len(test_envs) > 1 and target_env is not None:
                    train_env_str = ",".join(map(str, train_envs))
                    # filename = f"TE{target_env}_TR{train_env_str}_{step}_{args.setting_name}.pth"
                    filename = f"TE{target_env}_TR{train_env_str}_best_{args.setting_name}.pth"
                ...
                tmp_records = Q(records)
                tmp_tr_val_best_index = tmp_records.argmax("train_out", 1)
                tmp_tr_best = tmp_records[tmp_tr_val_best_index]["train_out"]
                print(tmp_tr_best)
                if results['train_out'][1] >= tmp_tr_best[1]:
                    print('saving path:',path)
                    filename = "TE{}_{}.pth".format(test_env_str, args.setting_name)
                    path = ckpt_dir / filename
                    print('saving path:', path)
            ...
            if swad:
                ...
                # swad_algorithm_for_save = swad.get_final_model()
                # ckpt_dir = args.out_dir / f"checkpoints_{step}"
                # ckpt_dir.mkdir(exist_ok=True)
                #
                # test_env_str = ",".join(map(str, test_envs))
                ...
                #     "model_hparams": dict(hparams),
                #     "test_envs": test_envs,
                #     "model_dict": swad_algorithm_for_save.cpu().state_dict(),
                # }
                # # algorithm = nn.DataParallel(algorithm)
                # # algorithm = algorithm.module
                # # if not args.debug:
                # torch.save(save_dict, path)
                # print(f'test env {test_envs} model saved at {path}')
    ...
    tr_val_best_index = records.argmax("train_out", 1)
    tr_val_best = records[tr_val_best_index]["test_in"]
    ...
    ret = {
        ...
        "training-domain validation (inD)": records[tr_val_best_index]["train_out"],
        #  "last": last,
