...
do
  if [ "$3" = "base" ]; then
        ...
        PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp-name no_gs_fixd_${SEED} \
                ...
                --nepochs 160 --batch-size 128 --results-path "$PROJECT_DIR/cifar100_baseline/$5base_$6tasks" \
                --approach $1 --gpu $2 --lr 0.1 --lr-factor 10 --momentum 0.9 \
                --weight-decay 5e-4 \
                --nc-first-task $5 \
                --num-exemplars-per-class 20 --exemplar-selection herding \
                --pl-classification-w 0. --pl-mask-entropy-w 0. --l-reg-w 0. # LT ??
#                --pl-classification-w 0.1 --pl-mask-entropy-w 1e-6 --l-reg-w 1e-3 # LT ??

    elif [ "$3" = "imagenet_subset" ]; then
        PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp-name no_gs_fixd_${SEED} \
                --datasets "$3_$4" --num-tasks $6 --network resnet18 --seed $SEED \
                --nepochs 90 --batch-size 128 --results-path "$PROJECT_DIR/imagenet_subset_baseline/$5base_$6tasks" \
                --approach $1 --gpu $2 --lr 0.1 --lr-factor 10 --momentum 0.9 \
                --weight-decay 1e-4 \
