...
def main():
    ...
    # optimizer = optim.Adam(pim.parameters(), lr=0.01, weight_decay=1e-4)
    optimizer = optim.Adam(pim.parameters(), lr=0.001, weight_decay=1e-2)
    ...
    for epoch in range(args.epochs):  # loop over the feature map set multiple times
        ...
        for mb_id in range(0, int(len(feature_map_preds) / mb_size)):
            ...
            with torch.no_grad():
                ...
                # print(targets_u)
                # targets_u = torch.softmax(targets_u, dim=1)
                targets_u_novel = targets_u[:, seen_N:]
                max_pred_novel, _ = torch.max(targets_u_novel, dim=-1)
                hard_novel_idx1 = torch.where(max_pred_novel >= args.threshold)[0]
                targets_u[hard_novel_idx1] = targets_u[hard_novel_idx1].ge(args.threshold).float()
                targets_u = torch.softmax(targets_u, dim=1)

                # targets_s = sinkhornsoft_mb_logits_outputs_seen_preds[:, seen_N:]
                # max_pred_novel2, _ = torch.max(targets_s, dim=-1)
                # hard_novel_idx2 = torch.where(max_pred_novel2 > args.threshold)[0]
                # ratio = (len(soft_mb_logits_outputs) - len(hard_novel_idx2))/len(soft_mb_logits_outputs)
                # targets_s[hard_novel_idx2] = targets_s[hard_novel_idx2].ge(args.threshold).float()
                # targets_s = torch.softmax(targets_s, dim=1)
            ...
            # # loss += criterion(soft_mb_logits_outputs_seen_preds[:, seen_N:], mb_lab_points.to(torch.int64)[torch.where(mb_lab_points.to(torch.int64)>=seen_N)])
            # # loss += criterion(mb_logits_outputs_seen_preds[hard_novel_idx2],  mb_lab_points.to(torch.int64)[hard_novel_idx2]) * len(
            # #     hard_novel_idx2) / len(mb_inputs) * auto_lambda_val
            # # print(mb_logits_outputs_unseen_preds[hard_novel_idx1].shape)
            #
            # y_ = torch.argmax(targets_u[hard_novel_idx1], dim=1).to(torch.int64)
            # hard_novel = mb_logits_outputs_unseen_preds[hard_novel_idx1]
            # loss += criterion(hard_novel, y_) * (len(
            #     hard_novel) / len(mb_inputs)) * auto_lambda_val
            #
            # ratio = auto_lambda_val * (len( hard_novel) / len(mb_inputs))
            # loss  += -((targets_u+ 1e-12) * torch.log(soft_mb_logits_outputs_unseen_preds+ 1e-12)).sum(1).mean() * ratio

            # #################################################################################
            # # imagenet_100, herbarium
            # loss = - ((soft_mb_logits_outputs_unseen_preds + 2.220446049250313e-16) * torch.log(
            #     soft_mb_logits_outputs_unseen_preds + 2.220446049250313e-16)).sum(1).mean() * auto_lambda_val
            # loss += ((soft_mb_logits_outputs.mean(0)) * torch.log(soft_mb_logits_outputs.mean(0) + 1e-12)).sum()
            # loss += criterion(mb_logits_outputs_seen_preds, mb_lab_points.to(torch.int64))
            ...
            #
            # ratio = auto_lambda_val * (len(hard_novel) / len(mb_inputs))
            # loss += -((targets_u + 1e-12) * torch.log(soft_mb_logits_outputs_unseen_preds + 1e-12)).sum(
            #     1).mean() * ratio * 0.1
            # #################################################################################
            ...
            y_ = torch.argmax(hard_novel, dim=1)
            y_ = F.one_hot(y_.to(torch.int64), num_classes=assumed_k)
            w = soft_mb_logits_outputs[hard_novel_idx1] * y_
            w = w.sum(-1) * torch.log(w.sum(-1))
            loss -= w.mean()  * len(y_) / len(mb_inputs) * auto_lambda_val



            if args.dataset == 'scars':
                ratio = (len( hard_novel) / len(mb_inputs)) * auto_lambda_val * 4 # 5
            elif args.dataset == 'cub':
                ratio = (len(hard_novel) / len(mb_inputs)) * auto_lambda_val * 0.55 # 0.55 # 0.5
            elif args.dataset == 'herbarium':
                ratio = (len(hard_novel) / len(mb_inputs)) * auto_lambda_val * 0.1
            elif args.dataset == 'cifar100':
                ratio = (len(hard_novel) / len(mb_inputs)) * auto_lambda_val * 5
            elif args.dataset == 'cifar10':
                ratio = (len(hard_novel) / len(mb_inputs)) * auto_lambda_val * 1.2 # 1
            elif args.dataset == 'imagenet_100':
                ratio = (len(hard_novel) / len(mb_inputs)) * auto_lambda_val * 0.1
            else:
                ratio = (len(hard_novel) / len(mb_inputs)) * auto_lambda_val
            ...
            # loss += -((targets_u + 1e-12) * torch.log(soft_mb_logits_outputs_unseen_preds + 1e-12)).sum(
            #     1).mean()  * (len(
            #     hard_novel) / len(mb_inputs))

            # cub
            # loss += -((targets_u + 1e-12) * torch.log(soft_mb_logits_outputs_unseen_preds + 1e-12)).sum(
            #     1).mean() * (len(hard_novel) / len(mb_inputs)) * auto_lambda_val
            # #################################################################################


            # loss -= ((soft_mb_logits_outputs_seen_preds[hard_novel_idx2] + 2.220446049250313e-16) * torch.log(
            #     soft_mb_logits_outputs_seen_preds[hard_novel_idx2] + 2.220446049250313e-16)).sum(1).mean() * auto_lambda_val
            # y_ = torch.argmax(mb_logits_outputs_unseen_preds, dim=1)
            # y_  = torch.argmax(targets_u, dim=1)
            # y_seen = mb_lab_points.to(torch.int64)
            # y_unseen_hard_novel = torch.argmax(hard_novel, dim=1)
            # reg = 0.
            # for i in range(assumed_k):
            #     # x_p = soft_mb_logits_outputs_seen_preds[y_seen == i]
            #     # # x_p_y_k = x_p.mean(0)
            #     # x_p_y_k = x_p
            #     # h_x_p_cond_y = ((x_p_y_k + 2.220446049250313e-16) * torch.log( x_p_y_k + 2.220446049250313e-16)).sum(1).mean() * len(x_p_y_k)/len(soft_mb_logits_outputs)
            #     # reg -=  0. if torch.isnan(h_x_p_cond_y) else h_x_p_cond_y * auto_lambda_val * 0.1
            #
            #     x_p = mb_logits_outputs_unseen_preds[hard_novel_idx1][y_unseen_hard_novel == i]
            #     # x_p_y_k = x_p.mean(0)
            #     x_p_y_k = x_p
            #     h_x_p_cond_y = ((x_p_y_k + 2.220446049250313e-16) * torch.log(x_p_y_k + 2.220446049250313e-16)).sum(
            #         1).mean() * len(x_p_y_k)/len(soft_mb_logits_outputs)
            #     reg -= 0. if torch.isnan(h_x_p_cond_y) else h_x_p_cond_y * auto_lambda_val
            #     # x_p = soft_mb_logits_outputs[~mb_lab_mask][y_ != i]
            #     # x_p_y_k = x_p.mean(0)
            #     # h_x_p_cond_y2 = ((x_p_y_k + 1e-12) * (torch.log(x_p_y_k) + 1e-12)).mean() * auto_lambda_val * 0.1
            ...
            # w = soft_mb_logits_outputs[hard_novel_idx1] * y_
            # w = w.sum(-1) * (w.sum(-1))
            # loss += w.mean() * auto_lambda_val * len(y_)/len(mb_inputs)



            # y_ = torch.argmax(targets_s[hard_novel_idx2], dim=1)
            # y_ = F.one_hot(y_.to(torch.int64), num_classes=assumed_k)
            # w = soft_mb_logits_outputs[hard_novel_idx2] * y_
            # w = w.sum(-1) * (1 - w.sum(-1))
            # loss -= w.mean() * auto_lambda_val * len(y_) / len(mb_inputs)

            # y_ = F.one_hot(mb_lab_points.to(torch.int64),  num_classes=assumed_k).detach()
            # w2 = mb_logits_outputs_seen_preds * y_
            # w2 = w2.sum(-1) * (1 - w2.sum(-1))
            # loss += w2.mean() * auto_lambda_val * len(y_)/len(mb_inputs)
            ######################################################
            # with torch.no_grad():
            #     y = soft_mb_logits_outputs.detach()
            #     y =  F.one_hot(torch.argmax(y, dim=1), num_classes=assumed_k)
            # w = soft_mb_logits_outputs * y
            # loss += w.mean() * auto_lambda_val
