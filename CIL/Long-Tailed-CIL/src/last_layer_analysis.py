...
def mask_layer_analysis(heads, task, taskcla, y_lim=False, sort_weights=False):
    """Plot last layer weight and bias analysis"""
    print('Plotting last layer analysis...')
    # num_classes = sum([x for (_, x) in taskcla])
    ...
    with torch.no_grad():
        for t in range(task + 1):
            n_classes_t = taskcla[t][1]
            weights.append((heads[t].linear1.weight ** 2).sum(1).sqrt().detach().cpu().numpy())
            if type(heads[t].linear1) == torch.nn.Linear:
                if heads[t].linear1.bias is not None:
                    biases.append(heads[t].linear1.bias.detach().cpu().numpy())
            else:
                biases.append(np.zeros(weights[-1].shape))  # For LUCIR
            num_dim = len(weights[-1])
            indexes.append(np.arange(0, num_dim))
            # class_id += n_classes_t
    ...
    for i, (x, y) in enumerate(zip(indexes, weights), 0):
        if sort_weights:
            ax.bar(x, sorted(y, reverse=True), label="Task {}".format(i), alpha=0.5)
        else:
            ax.bar(x, y, label="Task {}".format(i), alpha=0.5)
    ax.set_xlabel("Classes", fontsize=11, fontfamily='serif')
    ax.set_ylabel("Weights L2-norm", fontsize=11, fontfamily='serif')
    if num_dim is not None:
        ax.set_xlim(0, num_dim)
    if y_lim:
        ax.set_ylim(0, 5)
    ax.legend(loc='upper left', fontsize='11') #, fontfamily='serif')
    ...
    return f_weights, f_biases, weights
