...
def main():
    ...
    for epoch in range(args.epochs):  # loop over the feature map set multiple times
        ...
        for mb_id in range(0, int(len(feature_map_preds) / mb_size)):
            ...
            mb_logits_outputs = pim(mb_inputs, args.use_affine)
            ...
            loss = h_u_loss + + ce_l_loss + h_u_l_cond_z
            ...
            if args.h_u_cond_l_loss:
                h_u_cond_l_loss = - (
                    ...
                    1).mean()
                loss += h_u_cond_l_loss * auto_lambda_val * ratio

            if args.H_labelled:
                H_labelled = -((targets_l + e) * torch.log(soft_mb_logits_outputs_seen_preds + e)).sum(
                    1).mean()
                loss += H_labelled * auto_lambda_val * ratio

            if args.H_unlabelled:
                H_unlabelled = ((targets_u + e) * torch.log(soft_mb_logits_outputs_unseen_preds + e)).sum(
                    1).mean()
                loss += H_unlabelled * auto_lambda_val * ratio
