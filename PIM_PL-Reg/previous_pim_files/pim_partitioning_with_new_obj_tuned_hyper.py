...
def main():
    ...
    if args.dataset == 'scars':
        ...
        r = 1e-2
    optimizer = optim.Adam(pim.parameters(), lr=0.001, weight_decay=r)

    # optimizer = optim.Adam(pim.parameters(), lr=0.0001, weight_decay=r)

    # optimizer = optim.AdamW(pim.parameters(), lr=0.001, weight_decay=2e-2)
    # optimizer = optim.SGD(pim.parameters(), lr=0.001, weight_decay=1e-3)
    # optimizer = optim.Adam(pim.parameters(), lr=0.0012, weight_decay=1e-2)
    ...
    for epoch in range(args.epochs):  # loop over the feature map set multiple times
        ...
        for mb_id in range(0, int(len(feature_map_preds) / mb_size)):
            ...
            h_u_cond_l_loss = - ((soft_mb_logits_outputs_unseen_preds[hard_u_cond_l_index] + 2.220446049250313e-16) * torch.log(
                soft_mb_logits_outputs_unseen_preds[hard_u_cond_l_index] + 2.220446049250313e-16)).sum(1).mean() * auto_lambda_val * ratio
            ce_l_loss = criterion(mb_logits_outputs_seen_preds, mb_lab_points.to(torch.int64))
            h_u_certain_loss = 0
            h_u_l_cond_y = (soft_mb_logits_outputs.mean(0) * torch.log(soft_mb_logits_outputs.mean(0) + 1e-12)).sum()


            # mul_l_u_loss = 0.
            # n1 = len(mb_logits_outputs_unseen_preds)
            # n2 = len(mb_logits_outputs_seen_preds)
            # if n1>= n2:
            #     selected_n1 = torch.randint(0, n1, (n2,))
            #     mb_logits_outputs_unseen_preds = mb_logits_outputs_unseen_preds[selected_n1]
            # else:
            #     selected_n2 = torch.randint(0, n2, (n1,))
            #     mb_logits_outputs_seen_preds = mb_logits_outputs_seen_preds[selected_n2]
            #
            # # mul_l_u_loss = get_cond_shift(mb_logits_outputs_unseen_preds,mb_logits_outputs_seen_preds) * 0.001
            # mul_l_u_loss = get_cond_shift(mb_logits_outputs_seen_preds, mb_logits_outputs_unseen_preds, )
            # ratio = 1
            # if args.dataset == 'scars':
            #     ratio = 0.001
            # elif args.dataset == 'cub':
            ...
            # else:
            #     ratio = 0.001
            # mul_l_u_loss = mul_l_u_loss * ratio

            # loss = h_u_loss + h_l_loss + h_cond + (h_l_cond_y_loss + h_u_cond_l_y_loss)/2 + mul_l_u_loss + h_u_certain_loss
            loss = h_u_cond_l_loss + h_u_loss+ + ce_l_loss + h_u_l_cond_y + h_u_certain_loss
            # print(h_u_cond_l_loss.item(), h_l_loss.item(), h_u_certain_loss.item())
            ...
            # hard_novel = mb_logits_outputs_unseen_preds[hard_novel_idx]
            # if args.dataset == 'scars':
            #     ratio = (len( hard_novel) / len(mb_inputs)) * auto_lambda_val * 1 # 5
            # elif args.dataset == 'cub':
            ...
            # elif args.dataset == 'imagenet_100':
            #     ratio = (len(hard_novel) / len(mb_inputs)) * auto_lambda_val * 0.05
            # else:
            ...
            # w = soft_mb_logits_outputs[hard_novel_idx] * y_
            ...
            # print(h_u_loss.item(), h_l_loss.item(),  h_l_cond_y_loss.item(), h_u_cond_l_y_loss.item(),
            #       mul_l_u_loss.item())
            # print(h_cond.item())
