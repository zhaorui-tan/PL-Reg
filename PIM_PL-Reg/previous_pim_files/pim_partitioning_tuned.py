...
def main():
    ...
    # weight_decays = [1e-3, 2e-3,  5e-3, 1e-2, 2e-2, 5e-2]
    # weight_decays = [1e-2]
    # for w in weight_decays:
    # r = 1e-2
    optimizer = optim.Adam([
        {'params': pim.parameters()},
        # {'params': affine.parameters()},
                            ], lr=0.001/2, weight_decay=r)
    # optimizer_affine = optim.Adam(affine.parameters(), lr=0.001, weight_decay=r)
    print(f'using weight decay {r}')
    # optimizer = optim.Adam(pim.parameters(), lr=0.001, weight_decay=r)
    # optimizer = optim.Adam(pim.parameters(), lr=0.002, weight_decay=1e-3)
    # optimizer = optim.AdamW(pim.parameters(), lr=0.001, weight_decay=r)
    # optimizer = optim.Adam(pim.parameters(), lr=0.01, weight_decay=1e-3)
    ...
    for epoch in range(args.epochs):  # loop over the feature map set multiple times
        ...
        for mb_id in range(0, int(len(feature_map_preds) / mb_size)):
            ...
            loss = h_u_cond_l_loss + h_u_loss+ + ce_l_loss + h_u_l_cond_y
            # loss =   ce_l_loss + h_u_loss + h_u_l_cond_y
            H_labelled =  -((targets_l + 1e-12) * torch.log(soft_mb_logits_outputs_seen_preds + 1e-12)).sum(
                1).mean()
            loss += H_labelled

            H_unlabelled =  -((targets_u + 1e-12) * torch.log(soft_mb_logits_outputs_unseen_preds + 1e-12)).sum(
                1).mean() * auto_lambda_val * ratio
            ...
            # sample_for_visual(soft_mb_logits_outputs_seen_preds, soft_mb_logits_outputs_unseen_preds, mb_lab_points,
            #                   unseen_lab)
