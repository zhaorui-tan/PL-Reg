...
def main():
    ...
    for epoch in range(args.epochs):  # loop over the feature map set multiple times
        ...
        for mb_id in range(0, int(len(feature_map_preds) / mb_size)):
            ...
            with torch.no_grad():
                targets_u = sinkhorn(soft_mb_logits_outputs_unseen_preds) # hat(y)^
                ...
                #
                # # max_pred_novel = max_pred_novel[:, seen_N:]
                # # print(torch.where(max_pred_novel >= seen_N)[0])
                # # max_pred_novel = max_pred_novel[torch.where(max_pred_novel >= seen_N)[0]]
                # # print(max_pred_novel.shape)
                ...
                max_pred_unseen, _ = torch.max(targets_u, dim=-1)
                hard_unseen_idx = torch.where(max_pred_unseen >= args.threshold)[0]
                targets_u[hard_unseen_idx] = targets_u[hard_unseen_idx].ge(args.threshold).float()

                targets_u_novel = targets_u[:, seen_N:]
                max_pred_novel, _ = torch.max(targets_u_novel, dim=-1)
                # print(max_pred_novel.max(), max_pred_novel.min())
                hard_novel_idx1 = torch.where(max_pred_novel >= (args.threshold))[0]
                # targets_u[hard_novel_idx1] = targets_u[hard_novel_idx1].ge(args.threshold).float()

                combine_ = torch.cat([hard_novel_idx1, hard_unseen_idx], dim=-1)
                combine_ = combine_.unique()
                targets_u = torch.softmax(targets_u, dim=1)
                # print(targets_u)
            ...
            # loss += - ((soft_mb_logits_outputs_seen_preds + 2.220446049250313e-16) * torch.log(
            #     soft_mb_logits_outputs_seen_preds + 2.220446049250313e-16)).sum(1).mean() * auto_lambda_val
            #

            # loss += - ((soft_mb_logits_outputs_unseen_preds[~hard_unseen_idx] + 2.220446049250313e-16) * torch.log(
            #     soft_mb_logits_outputs_unseen_preds[~hard_unseen_idx] + 2.220446049250313e-16)).sum(1).mean() * auto_lambda_val
            #
            # loss += - ((soft_mb_logits_outputs_unseen_preds[~hard_novel_idx1] + 2.220446049250313e-16) * torch.log(
            #     soft_mb_logits_outputs_unseen_preds[~hard_novel_idx1] + 2.220446049250313e-16)).sum(
            #     1).mean() * auto_lambda_val

            loss += - ((soft_mb_logits_outputs_unseen_preds[~hard_unseen_idx] + 2.220446049250313e-16) * torch.log(
                soft_mb_logits_outputs_unseen_preds[~hard_unseen_idx] + 2.220446049250313e-16)).sum(
                1).mean() * auto_lambda_val
            ...
            #     hard_novel) / len(mb_inputs)) * auto_lambda_val # unseen hard novel entropy

            # ratio = auto_lambda_val
            if args.dataset == 'scars':
                ratio = (len( soft_mb_logits_outputs_unseen_preds) / len(mb_inputs)) * auto_lambda_val * 4 # 5
            elif args.dataset == 'cub':
                ratio = (len(soft_mb_logits_outputs_unseen_preds) / len(mb_inputs)) * auto_lambda_val * 0.55 # 0.55 # 0.5
            elif args.dataset == 'herbarium':
                ratio = (len(soft_mb_logits_outputs_unseen_preds) / len(mb_inputs)) * auto_lambda_val * 0.1
            elif args.dataset == 'cifar100':
                ratio = (len(soft_mb_logits_outputs_unseen_preds) / len(mb_inputs)) * auto_lambda_val * 0.5
            elif args.dataset == 'cifar10':
                ratio = (len(soft_mb_logits_outputs_unseen_preds) / len(mb_inputs)) * auto_lambda_val * 1.2 # 1
            elif args.dataset == 'imagenet_100':
                ratio = (len(soft_mb_logits_outputs_unseen_preds) / len(mb_inputs)) * auto_lambda_val * 0.1
            else:
                ratio = (len(soft_mb_logits_outputs_unseen_preds) / len(mb_inputs)) * auto_lambda_val


            loss += ((targets_u[hard_unseen_idx] + 1e-12) * torch.log(soft_mb_logits_outputs_unseen_preds[hard_unseen_idx] + 1e-12)).sum(
                1).mean() * ratio # unseen entropy

            # loss += ((targets_u[hard_novel_idx1] + 1e-12) * torch.log(soft_mb_logits_outputs_unseen_preds[hard_novel_idx1] + 1e-12)).sum(
            #     1).mean() * ratio  # unseen entropy
            #
            # loss += ((targets_u[hard_unseen_idx] + 1e-12) * torch.log(
            #     soft_mb_logits_outputs_unseen_preds[hard_unseen_idx] + 1e-12)).sum(
            #     1).mean() * ratio  # unseen entropy
