...
def main(argv=None):
    ...
    parser.add_argument('--batch-size', default=1, type=int, required=False,
                        help='Number of samples per batch to load (default=%(default)s)')
    parser.add_argument('--num-tasks', default=4, type=int, required=False,
                        help='Number of tasks per dataset (default=%(default)s)')
    parser.add_argument('--schedule_step', default=[250,350,450], nargs = '+' ,type = int,required=False,
                        help='scheduler steps (default=%(default)s)') ### [100,200,250], [80,120], [250,350,450]
    ...
    parser.add_argument('--eval-on-train', action='store_false',
    ...
    # PL-logic
    parser.add_argument('--pl-classification-w', default=0.001, type=float,
                        help='weight of partial logic reg classification')
    parser.add_argument('--pl-mask-entropy-w', default=0.001, type=float,
                        help='weight of partial logic reg mask diversity')
    parser.add_argument('--l-reg-w', default=0.001, type=float,
                        help='weight of partial logic reg l-reg')
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        # Train
        # pl_classification_w = 0.000 #0.001
        # pl_mask_entropy_w = 0.000 #0.001
        pl_classification_w = args.pl_classification_w
        pl_mask_entropy_w = args.pl_mask_entropy_w
        l_reg_w = args.l_reg_w
        appr.train(t, trn_loader[t], tst_loader[t], args, tst_loader, pl_classification_w=pl_classification_w,pl_mask_entropy_w=pl_mask_entropy_w, l_reg_w=l_reg_w)
        print('-' * 108)

        # Test
        for u in range(t + 1):
            test_loss, acc_taw[t, u], acc_tag[t, u] = appr.eval(u, tst_loader[u],  pl_classification_w=pl_classification_w,pl_mask_entropy_w=pl_mask_entropy_w, l_reg_w=l_reg_w)
            if u < t:
