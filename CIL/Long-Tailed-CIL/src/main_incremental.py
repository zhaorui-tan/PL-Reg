...
from last_layer_analysis import last_layer_analysis, mask_layer_analysis
...
def main(argv=None):
    ...
    utils.seed_everything(seed=args.seed)
    appr = Appr(net, device, **appr_kwargs, args=args)
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        # weights.savefig(f'vis/weights_{t}.png')
        # biases.savefig(f'vis/biases_{t}.png')
        logger.log_figure(name=f'weights_{t}', iter=t, figure=weights)

        # Output sorted weights and biases
        weights, biases = last_layer_analysis(net.heads, t, taskcla, y_lim=True, sort_weights=True)
        # weights.savefig(f'vis/sorted_weights_{t}.png')
        # biases.savefig(f'vis/sorted_biases_{t}.png')
        logger.log_figure(name=f'sorted_weights_{t}', iter=t, figure=weights)


        weights, biases, weights_ = mask_layer_analysis(net.partial_logic_mask, t, taskcla, y_lim=True)
        # weights.savefig(f'vis/mask_weights_{t}.png')
        logger.log_figure(name=f'sorted_weights_{t}', iter=t, figure=weights)
        print(weights, np.array(weights))
        # np.save(f'vis/weights_{t}.npy', np.array(weights_))
        # biases.savefig(f'vis/mask_biases_{t}.png')
        logger.log_numpy_arr(name=f'weights_{t}', array=np.array(weights_))
