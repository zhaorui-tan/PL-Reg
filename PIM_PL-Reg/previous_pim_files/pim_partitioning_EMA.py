...
def get_arguments():
    ...
    parser.add_argument("--seed", type=int, default=2023, help="Seed for RandomState.")
...
def main():
    ...
    for epoch in range(args.epochs):  # loop over the feature map set multiple times
        ...
        if epoch % 100 == 0:
            ...
            with torch.no_grad():
                ...
                unlab_preds = np.asarray(
                    from_torch_to_numpy(predicted)[~mask_lab], dtype=int
                )
    ...
    # Before evaluation, apply EMA parameters to the model
    ema.apply_shadow()
    ...
    np.save(
        f"./logits/{args.dataset}/mb_logits_outputs_seen_preds.npy",
        mb_logits_outputs_seen_preds.detach().cpu().numpy(),
    )
    np.save(
        f"./logits/{args.dataset}/mb_logits_outputs_unseen_preds.npy",
        mb_logits_outputs_unseen_preds.detach().cpu().numpy(),
    )
