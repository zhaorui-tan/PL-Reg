...
class Appr(Inc_Learning_Appr):
    ...
    # add exemplars to train_loader
            ...
            for e in range(self.nepochs):
                # Train
                clock0 = time.time()
                # print("analyzing loader")
                # imgss = trn_loader.dataset.images
                # targets_test = trn_loader.dataset.labels
                # print("images shape", imgss.shape)
                # breakpoint()
                # print(c)
    ...
    def train_epoch_stage2(self , t,train_loader):
        ...
        for i, (images, target) in enumerate(train_loader):
            # print(i, torch.unique(target, return_counts=True))
            if i > end_steps:
                break
            with torch.no_grad():
...
class CosineLinear(Module):
    ...
    def forward(self, input, stochastic=None, cs_stoc=None, global_variance=None):
