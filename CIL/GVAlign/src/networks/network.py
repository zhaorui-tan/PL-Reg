...
class LLL_Net(nn.Module):
    ...
    def forward(self, x, return_features=False, stochastic=False, cs_stoc=None, manifold_mixup=None, layer_mix=None,
                target=None, lamda_norm_list=None, task=None, pl_classification_w=None, pl_mask_entropy_w=None, return_PL=True):
        ...
        # pl_classification_w = 0.0001
        # pl_mask_entropy_w = 0.0001
        if manifold_mixup:
            # print('manifold_mixup')
            x, y_a, y_b, lam = self.model(x, manifold_mixup=manifold_mixup, layer_mix=layer_mix, target=target,
            ...
            for head_idx in range(len(self.heads)):
                if task:
                    mask = self.partial_logic_mask[task](x)
                else:
                ...
                if return_PL:
                    ...
                    PL_true = self.partial_logic_classifier[head_idx](x)
                ...
                if stochastic:
                    ...
                    y.append(head(x, stochastic))
            # print()
            # print('manifold_mixup', y)
            if return_PL:

                if return_features:
                    return y, x, y_a, y_b, lam, PL_loss
                else:
                    return y, y_a, y_b, lam, PL_loss
            else:
                if return_features:
            ...
            for head_idx in range(len(self.heads)):
                ...
                # print('mask.shape,x.shape', mask.shape,x.shape)
                x = x * mask
    ...
    def forward_int_nodes(self, x, return_features=False, stochastic=False, cs_stoc=None, manifold_mixup=None,
                          int_nodes_output=None):
        """Applies the forward pass
