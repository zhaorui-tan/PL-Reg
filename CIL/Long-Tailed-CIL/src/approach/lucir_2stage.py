...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=160, lr=0.1, lr_min=1e-4, lr_factor=10, lr_patience=8, clipgrad=10000,
                 ...
                 remove_less_forget=False, remove_margin_ranking=False, remove_adapt_lamda=False, args=None):
        ...
        if args is not None:
            self.pl_classification_w = args.pl_classification_w
            self.pl_mask_entropy_w = args.pl_mask_entropy_w
            self.l_reg_w= args.l_reg_w
            print( self.pl_classification_w, self.pl_mask_entropy_w, self.l_reg_w)
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            # Forward current model
            outputs, features, PL_loss = self.model(images, return_features=True,
                                                    pl_classification_w=self.pl_classification_w,
                                                    pl_mask_entropy_w=self.pl_mask_entropy_w,
                                                    # l_reg_w=self.l_reg_w
                                                    )
            lwsoutputs=[]
            ref_outputs = None
            ref_features = None
            if t > 0:
                ref_outputs, ref_features, _ = self.ref_model(images, return_features=True)
    ...
    def train_epoch_stage2(self , t,train_loader):
        ...
        for i, (images, target) in enumerate(train_loader):
            ...
            for idx in range(len(self.model.heads)):
                ...
                if self.pl_classification_w >0:

                    # with torch.no_grad():
                    mask = self.model.partial_logic_mask[idx](feat.detach())
                    ...
                    PL_loss += nn.CrossEntropyLoss()(pl, pl_target) * self.pl_classification_w
                    ...
                    PL_loss += -mask_entropy * self.pl_mask_entropy_w  #self.use_partial
                    # PL_loss += -mask_entropy * 0.1  # self.use_partial
                else:
                    tmp_feat = feat

                output.append(self.model.heads[idx](tmp_feat))
    ...
    def criterion(self, t, outputs, targets, ref_outputs=None, features=None, ref_features=None,stage2 = False,lwsoutputs = []):
        ...
        if ref_outputs is None or ref_features is None or stage2:
            ...
            #
            #     # features_ = features / features.norm(dim=-1, keepdim=True)  # normalize at the first
            #     # B = features_.T @ features_
            #     # id_matrix = torch.eye(*B.size(), out=torch.empty_like(B)).cuda()
            #     # ortho_reg = nn.MSELoss()(id_matrix, B)
            #     # L_reg = ortho_reg * 0.01
            #     L_reg = (A_entropy+A_dim_entropy)*0.01
            #
            # else:
            #     L_reg = 0
            ...
            if self.margin_ranking:
                ...
                if hard_num > 0:
                    ...
                    loss_mr = nn.MarginRankingLoss(margin=self.dist)(
                        gt_scores.view(-1, 1),
                        max_novel_scores.view(-1, 1),
                        torch.ones(hard_num * self.K, 1).to(self.device)  # Reshape to match input dimensions
                    )
            ...
            if self.l_reg_w > 0:
                if features is not None:
                    ...
                    A = torch.matmul(torch.cat([o['wsigma'] for o in outputs], dim=1).permute(1, 0), features)
                    A = F.softmax(A, dim=1)
                    A_entropy = ((A.mean(0) + 1e-12) * torch.log(A.mean(0) + 1e-12)).sum()
                    A_dim_entropy = (-((A + 1e-12) * torch.log(A + 1e-12)).sum(1).mean())
                    ...
                    # ortho_reg = nn.MSELoss()(id_matrix, B)
                    # L_reg = ortho_reg * 0.01
                    L_reg = (A_entropy + A_dim_entropy) * self.l_reg_w
